...
public class Sort {
	...
	public static int binarySearch(int[] nums, int formIndex, int toIndex,
			int key) {
		int low = formIndex;
		int high = toIndex - 1;
	...
	public static void threeQuickSort(int[] array, int begin, int end) {
		...
		/*
		 * 工作指针 q指向序列左边等于pivot元素的位置 p指向序列右边等于Pivot元素的位置 low指向从左向右扫面时的元素
		 * high指向从右向左扫描时的元素
		 */
		int low = begin;
		int high = end;
		...
		/*
		 * 因为工作指针q指向的是当前需要处理元素的下一个元素 故而需要退回到当前元素的实际位置，然后将等于pivot元素交换到序列中间
		 */
		q--;
		while (q >= begin && q != low) {
			swap(array, q--, low--);
	...
	/*
	 * tox:目标数组的行号 toy:目标数组的列号 fromx:源数组的行号 fromy:源数组的列号 r:数组的大小为 r*r
	 */
	public static void Copy(int[][] schedule, int tox, int toy, int fromx,
			int fromy, int r) {
		for (int i = 0; i < r; i++) {
			for (int j = 0; j < r; j++) {
	...
	public static void main(String[] args) {
		...
		// int[] p = {1, 2, 1, 2, 1, 3, 3, 3,
		// 2};//{7,1,7,1,2,7,2,3,1,3,1,3,4,5,6,0,5,0,0,6};//
